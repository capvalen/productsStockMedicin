...
<head>
	...
	<title>presentacion Panel</title>
		...
		<div class="container-fluid px-5">
			...
			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav me-auto mb-2 mb-lg-0">
					...
					<li class="nav-item">
						<a class="nav-link active" href="presentaciones.php">Presentaciones</a>
					</li>
					<li class="nav-item">
	...
	<div class="container">
		...
		<div class="row col">
			<div class="d-grid gap-1 col-3 ms-auto">
				<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalNuevoProducto">Crear presentacion</button>
			</div>

		</div>
		<p>Lista de presentaciones:</p>
		<table class="table table-hover">
			...
			<tbody>
				<tr v-for="(presentacion, index) in presentaciones">
					<td>{{index+1}}</td>
					<td class="text-capitalize">{{presentacion.nombre}}</td>
					<td><button type="button" class="btn btn-outline-danger btn-sm border-0" @click="borrarProveedor(presentacion.id, presentacion.nombre, index)"><i class="bi bi-x-circle-fill"></i></button></td>
				</tr>
			</tbody>
				...
				<div class="modal-content">
					<div class="modal-body">
						<div class="d-flex justify-content-between mb-3">
							<h5 class="modal-title" id="exampleModalLabel">Nuevo presentacion</h5>
							<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
						</div>

						<div class="form-floating mb-3">
							<input type="email" class="form-control" id="floatingInput" placeholder=" " autocomplete="off" v-model="nombre">
							<label for="floatingInput">Nombre de presentacion</label>
						</div>
					
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-primary" @click="crearProveedor" data-bs-dismiss="modal">Crear presentacion</button>
					</div>
				</div>
			</div>
		</div>
	...
	var app=Vue.createApp({
		data() {
			return {
				...
				presentaciones:[], topicos:[], presentaciones:[],
		...
		methods:{
			...
			async crearProveedor(){
				...
				const respuesta = await fetch(this.servidor + 'crearPresentacion.php', {
					method: 'POST', body: datos
				});
				var idProveedor = await respuesta.text();
				if(!isNaN(idProveedor)){
					this.presentaciones.unshift({id:idProveedor, nombre: this.nombre })
				}
			},
			async borrarProveedor(queProveedor, name, queIndex){
				if(confirm('¿Desea eliminar el presentacion: '+name.toUpperCase()+'?')){
					var datos = new FormData();
					datos.append('id', queProveedor);
					const respEliminar = await fetch(this.servidor+'eliminarPresentacion.php',{ method:'POST', body:datos })
					var quePaso = await respEliminar.text();
					if( quePaso=='ok' ){
						this.presentaciones.splice(queIndex,1)
					}
				}
			},
